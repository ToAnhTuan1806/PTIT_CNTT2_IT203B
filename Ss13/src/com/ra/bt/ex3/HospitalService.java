...
Output:
Thông báo thành công khi hoàn tất cả 3 bước
Nếu có lỗi → thông báo thất bại và rollback dữ liệu

2. Đề xuất giải pháp
Sử dụng Connection của JDBC để quản lý Transaction
Tắt AutoCommit để gom các câu lệnh SQL vào cùng một giao dịch
Dùng try – catch để bắt lỗi
Nếu thành công → commit()
Nếu lỗi hoặc gặp bẫy dữ liệu → rollback() để đảm bảo dữ liệu không bị sai lệch

3. Thiết kế các bước xử lý
Mở kết nối database
Tắt AutoCommit
Lấy thông tin số dư bệnh nhân
